...
export const getShop = async (id, account = null) => {
    ...
    // save skin prices from store offers
    if (json.SkinsPanelLayout?.SingleItemStoreOffers) {
        for (const offer of json.SkinsPanelLayout.SingleItemStoreOffers) {
            const vpCost = offer.Cost?.[Object.keys(offer.Cost)[0]];
            if (vpCost) addPrice(offer.OfferID, vpCost);
        }
    }
...
export const getOffers = async (id, account = null) => {
    ...
    return await easterEggOffers(id, account, {
        ...
        storeOffers: resp.shop.SkinsPanelLayout.SingleItemStoreOffers,
...
const addShopCache = (puuid, shopJson) => {
    ...
    const shopCache = {
        offers: {
            ...
            storeOffers: shopJson.SkinsPanelLayout.SingleItemStoreOffers,
